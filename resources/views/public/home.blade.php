...
@section ('content')
    <section class="relative -mt-16 h-screen w-full overflow-hidden">
        <img
            src="{{ Vite::asset('resources/images/tultepec02.webp') }}"
            alt=""
            class="absolute inset-0 size-full object-cover"
        />
        <div class="absolute inset-0 bg-black/60"></div>
        <div
            class="relative mx-auto flex h-full max-w-6xl flex-col justify-center gap-8 px-4 text-white sm:px-6"
        >
            <h1
                class="max-w-3xl text-4xl leading-tight font-bold sm:text-5xl md:text-6xl"
            >
            ...
            <div class="font-display flex flex-wrap gap-4">
                ...
                <a
                    href="#"
                    class="cursor-pointer rounded-lg bg-white px-5 py-3 font-semibold text-black transition-colors hover:bg-gray-300"
    ...
    <section class="border-b border-gray-300">
        <div class="mx-auto flex max-w-6xl flex-col gap-10 px-4 py-16 sm:px-6">
            ...
            <div class="grid gap-5 md:grid-cols-3">
                ...
                @endphp
                    <div
                        class="relative flex flex-col gap-4 rounded-2xl border border-gray-300/50 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md"
                    >
                        <span
                            class="grid size-12 place-items-center rounded-xl {{ $bgColor }}"
                        >
                            <x-dynamic-component
                        ...
                        <a
                            href="{{ $feature['href'] }}"
                            class="{{ $txtColor }} font-semibold inline-flex items-center gap-1.5"
                        >
                            {{ $feature['link_text'] }}
    ...
    <section class="border-b border-gray-300 bg-sky-200/20">
        <div
            class="mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2"
        >
            <div
                class="relative aspect-4/3 overflow-hidden rounded-3xl border border-[#d6e0e4] shadow-sm"
            >
                <img
                    src="{{ Vite::asset('resources/images/tultepec03.webp') }}"
                    alt=""
            ...
            <div>
                ...
                <ul class="mt-8 grid gap-4 sm:grid-cols-2">
                    <li class="flex gap-3">
                        <span
                            class="mt-0.5 grid size-6 shrink-0 place-items-center rounded-full bg-sky-200"
                        >
        ...
        <div class="mx-auto flex max-w-6xl flex-col gap-10 px-4 py-16 sm:px-6">
            <div class="flex flex-wrap items-end justify-between gap-4">
                ...
                <a
                    ...
                    class="cursor-pointer rounded-lg border border-gray-300 bg-white px-3 py-2 font-semibold text-black transition-colors hover:bg-gray-100"
                    >Ver biblioteca</a
                >
            </div>
            <div class="grid gap-5 md:grid-cols-3">
                @foreach ($publications as $publication)
                    <div
                        class="group relative flex h-full flex-col rounded-2xl border border-gray-300/50 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md"
                    >
                        <div
        ...
        <div class="mx-auto flex max-w-6xl flex-col gap-10 px-4 py-16 sm:px-6">
            ...
            <ol class="grid gap-5 md:grid-cols-3">
                @foreach ($reportSteps as $reportStep)
                    <li
                        class="flex flex-col gap-4 rounded-2xl border border-gray-400/50 bg-white p-6"
                    >
                        <span
                            class="font-display text-sm font-semibold"
                            >{{ $reportStep['step'] }}</span
                        >
                        <span
                            class="grid size-10 place-items-center rounded-xl bg-sky-200"
                        >
                            <x-dynamic-component
                                :component="'icons.' . $reportStep['icon']"
                            />
                        </span>
                        <h3 class="font-semibold">
                            {{ $reportStep['title'] }}
                        </h3>
                        <p class="font-display text-sm">{{ $reportStep['description'] }}</p>
                    </li>
                @endforeach
            </ol>
            <div
                class="flex flex-col items-start gap-4 rounded-3xl bg-blue-950 p-8 text-white sm:flex-row sm:items-center sm:justify-between"
            >
                ...
                <a
                    href="{{ route('reports.create') }}"
                    class="cursor-pointer self-center rounded-lg bg-red-600 px-5 py-3 text-center font-semibold text-white transition-colors hover:bg-red-700"
                    >Reportar un riesgo</a
                >
            </div>
        </div>
